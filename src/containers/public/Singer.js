...
const { SlUserFollow, BsPlayFill } = icons
...
const Singer = () => {
    ...
    useEffect(() => {
        const fetchArtistProfie = async () => {
            const res = await apis.apiGetArtist(singer);
            if (res.data.err === 0) {
                setArtistData(res.data.data)
            }
        }
        singer && fetchArtistProfie();
    }, [singer])
    ...
    return (
        <div className='flex flex-col w-full pb-10'>
            <div ref={ref} className='relative'>
                <img src={artistData?.cover} className='w-full h-[410px] object-cover' alt='bg' />
                <div className='absolute left-[0] right-[0] bottom-0 top-[210px] bg-gradient-to-t from-[#0e131a] to-[rgba(14,19,26,0)] opacity-40'></div>
                <div className='absolute left-[0] right-[0] bottom-0 top-[110px] bg-gradient-to-b from-[rgba(0,0,0,0)] via-[rgba(0,0,0,0)] to-[rgba(76,79,64,0.8)]'></div>
                <div className='absolute left-0 right-0 bottom-0 top-[135px] text-white px-[59px] pb-6 flex justify-between'>
                    <div className='flex flex-col justify-end'>
                        <div className='flex gap-6 items-center mb-[16px]'>
                            <h1 className='text-[60px] font-[700]'>
                                {artistData?.name}
                            </h1>
                            <span className='p-3 rounded-full bg-white text-main-500 hover:bg-main-500 hover:text-gray-100 cursor-pointer'>
                                <BsPlayFill size={30} />
                            </span>
                        </div>
                        <div className='flex items-center gap-6'>
                            <span className='text-sm text-[#feffffcc]'>
                                {Number(artistData?.totalFollow.toFixed(1)).toLocaleString()} người quan tâm
                            </span>
                            <button type='button' className='bg-[rgba(254,255,255,.1)] border border-[rgba(254,255,255,.2)]  px-6 py-1 text-sm rounded-r-full rounded-l-full flex text-white gap-1 hover:opacity-100'>
                                <span>
            ...
            <div className='w-full px-[59px] mt-[30px] flex flex-col gap-5'>
                ...
                <div className='flex flex-wrap w-full justify-between'>
                    {
                        artistData?.sections[0]?.items?.filter((i, index) => index < 6).map((item, index) => (
                            <div key={item.encodeId} className='w-[49%]' >
            ...
            </div>
            {
                artistData?.sections?.filter(item => item.sectionType === 'playlist')?.map((item, index) => (
                    <Section key={index} data={item} isHideDesc isHideArtist />
                ))
            }

            <div className='flex flex-col px-[59px] mt-12'>
                <h3 className='text-lg font-bold mb-5'>
                    {artistData?.sections?.length === 7 ? artistData?.sections[6]?.title : artistData?.sections[5]?.title}
                </h3>
            ...
            <div className='flex flex-col px-[59px] mt-12'>
                ...
                <div className='flex gap-[30px]'>
                    <div className='w-[40%] m-h-[500px] rounded-md'>
                        <img src={artistData?.thumbnail} className='w-full rounded-md object-cover' />
                    </div>
                    <div className='w-[60%] flex flex-col gap-12'>
                        <span className='text-sm text-[#696969]' dangerouslySetInnerHTML={{ __html: artistData?.biography }}>
                        </span>
